...
import { AdminMasterServiceService } from '../../teams/selection-admission/adminMaster/Services/admin-master-service.service';

@Component({
  selector: 'app-admin-master-menu',
  templateUrl: './admin-master-menu.component.html',
  styleUrls: ['./admin-master-menu.component.scss'],
  animations: [
...
export class AdminMasterMenuComponent implements OnInit {
  ...
  constructor(public menuItems: MenuItems, private modalService: NgbModal,private US:AdminMasterServiceService,private route:Router) {
    this.navType = 'st5';
  ...
  user={email:'',nom:'',prenom:'',password:'',num_passport:'',age:0,cin:null,admnom:'',
  sexe:'',date_naissance:'',ville:null,gouvern_naissance:null,id_situation_professionnel:null,
  gouvernorat_adresse:null,pays:null,rue:'',code_postale:'',id_role:2};
  ngOnInit() {
   setTimeout(() => {
    this.US.getAdminbyId(Number(localStorage.getItem('id'))).subscribe(val=>{
      console.log(val);
      //@ts-ignore
      this.user=val.results[0];
    })
   }, 500);
